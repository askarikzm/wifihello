...
function FinanceDashboardContent() {
  ...
  const [period, setPeriod] = useState<'today' | 'week' | 'month'>(
    (searchParams.get('period') as 'today' | 'week' | 'month') || 'month'
  );
    ...
    const fetchFinanceData = async () => {
      ...
      try {
        ...
        if (overdueRes.ok) {
          ...
          setOverdueInvoices(overdueData.data || overdueData || []);
        }
      } catch (error) {
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card className="p-6">
          ...
          <div className="space-y-4">
            {['JazzCash', 'Easypaisa', 'Bank Transfer', 'Cash'].map((method, idx) => {
              const methodPayments = payments.filter(p => 
                p.gateway?.toLowerCase().includes(method.toLowerCase().replace(' ', ''))
              );
      ...
      <div className="grid gap-6 lg:grid-cols-2">
                ...
                <tbody>
                  {payments.slice(0, 8).map((payment) => (
                    <tr key={payment.id} className="border-b last:border-0 text-sm">
                      <td className="py-2.5">
                        <p className="font-medium text-slate-900">{payment.subscriber_name || 'Customer'}</p>
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <a href="/admin/invoices/overdue" className="text-sm text-blue-600 hover:text-blue-700">
              View all
            </a>
          </div>
                ...
                <tbody>
                  {overdueInvoices.slice(0, 8).map((invoice) => (
                    <tr key={invoice.id} className="border-b last:border-0 text-sm">
                      <td className="py-2.5">
                        <p className="font-medium text-slate-900">{invoice.subscriber_name || 'Customer'}</p>
                      </td>
